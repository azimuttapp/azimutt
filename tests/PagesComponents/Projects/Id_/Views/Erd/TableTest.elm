...
suite =
    describe "PagesComponents.Projects.Id_.Views.Erd.Table"
        [ describe "viewTable.argsToString"
            [ test "test 1"
                (\_ ->
                    Table.argsToString Platform.PC CursorMode.Drag "a" "b" 2 True False True False
                        |> Table.stringToArgs
                        |> Expect.equal ( ( Platform.PC, CursorMode.Drag ), ( "a", "b", 2 ), ( ( True, False ), ( True, False ) ) )
                )
            , test "test 2"
                (\_ ->
                    Table.argsToString Platform.Mac CursorMode.Select "c" "d" 4 False True False True
                        |> Table.stringToArgs
                        |> Expect.equal ( ( Platform.Mac, CursorMode.Select ), ( "c", "d", 4 ), ( ( False, True ), ( False, True ) ) )
                )
            ]
        ]
