...
suite =
    describe "AlterTable"
        [ describe "parseAlterTable"
            [ testStatement "primary key" "ALTER TABLE public.t2 ADD CONSTRAINT t2_id_pkey PRIMARY KEY (id);" parseAlterTable (\_ -> Ok (AddTableConstraint (Just "public") "t2" (ParsedPrimaryKey (Just "t2_id_pkey") (Nel "id" []))))
            ...
            , testStatement "without schema" "ALTER TABLE t2 ADD CONSTRAINT t2_id_pkey PRIMARY KEY (id);" parseAlterTable (\_ -> Ok (AddTableConstraint Nothing "t2" (ParsedPrimaryKey (Just "t2_id_pkey") (Nel "id" []))))
            , testStatement "with only" "ALTER TABLE ONLY public.t2 ADD CONSTRAINT t2_id_pkey PRIMARY KEY (id);" parseAlterTable (\_ -> Ok (AddTableConstraint (Just "public") "t2" (ParsedPrimaryKey (Just "t2_id_pkey") (Nel "id" []))))
            , testStatement "primary key with add" "ALTER TABLE public.t2 ADD PRIMARY KEY (`id`);" parseAlterTable (\_ -> Ok (AddTableConstraint (Just "public") "t2" (ParsedPrimaryKey Nothing (Nel "id" []))))
            , testStatement "bad" "bad" parseAlterTable (\_ -> Err [ "Can't parse alter table: 'bad'" ])
            ]
        , describe "parseAlterTableAddConstraintForeignKey"
            [ test "with on delete" (\_ -> "FOREIGN KEY (supply_order_bill_id) REFERENCES public.supply_order_bills(id) ON DELETE SET NULL" |> parseAlterTableAddConstraintForeignKey |> Expect.equal (Ok { column = "supply_order_bill_id", ref = { schema = Just "public", table = "supply_order_bills", column = Just "id" } }))
            ]
        ]
