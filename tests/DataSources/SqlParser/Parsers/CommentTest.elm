...
import DataSources.SqlParser.Parsers.Comment exposing (ParsedComment, parseColumnComment, parseTableComment)
    ...
    describe "Comment"
        ...
        , describe "parseColumnComment"
            [ testParse ( parseColumnComment, "basic" )
                ...
                { schema = Just "public", table = "table1", column = "col", comment = ParsedComment "A comment" }
            , testParse ( parseColumnComment, "with quotes" )
                "COMMENT ON COLUMN public.table1.col IS 'A ''good'' comment';"
                { schema = Just "public", table = "table1", column = "col", comment = ParsedComment "A 'good' comment" }
