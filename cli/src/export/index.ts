...
export async function exportDbSchema(kind: DbKind, url: DbUrl, opts: Opts): Promise<void> {
...
async function exportJsonSchema<T extends object>(kind: DbKind, url: DbUrl, opts: Opts, fetchSchema: (url: DbUrl, schema: string | undefined, sampleSize: number) => Promise<T>, transformSchema: (s: T, flatten: number, inferRelations: boolean) => AzimuttSchema, name: string) {
    if (opts.format !== 'json') {
