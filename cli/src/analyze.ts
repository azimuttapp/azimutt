...
export async function launchAnalyze(
  url: string,
  opts: Opts,
  logger: Logger
): Promise<void> {
  ...
  if (!connector)
    return Promise.reject(
      `Invalid connector for ${dbUrl.kind ? `${dbUrl.kind} db` : `unknown db (${dbUrl.full})`}`
    )
  if (opts.email && !isValidEmail(dbUrl, opts.email, logger))
    return Promise.reject(`Invalid email (${opts.email})`)
  if (opts.key && !isValidKey(dbUrl, opts.email, opts.key, logger))
    return Promise.reject(`Invalid key (${opts.key})`)
  if (opts.ignoreViolationsFrom && !opts.key)
    return Promise.reject(`You need a 'key' to ignore violations from a report`)

  // TODO: extend config for user, database, queries, data... ({user: {}, database: {}, queries: {}, data: {}, rules: {}})
  const app = "azimutt-analyze"
  const folder =
    opts.folder || `~/.azimutt/analyze${dbUrl.db ? "/" + dbUrl.db : ""}`
  const now = Date.now()
  const conf: RulesConf = await loadConf(folder, logger)
  const history = opts.key ? await loadHistory(folder, logger) : []
  const referenceReport: AnalyzeReport | undefined = opts.ignoreViolationsFrom
    ? await loadReferenceReport(folder, opts.ignoreViolationsFrom, logger)
    : undefined
  const connectorLogger = conf.database?.logQueries ? logger : loggerNoOp
  const db: Database = await connector.getSchema(app, dbUrl, {
    ...conf.database,
    logger: connectorLogger,
  })
  const queries: DatabaseQuery[] = await connector
    .getQueryHistory(app, dbUrl, {
      database: dbUrl.db,
      logger: connectorLogger,
    })
    .catch((err) => {
      if (typeof err === "string" && err === "Not implemented")
        logger.log(
          chalk.blue(
            `Query history is not supported yet on ${dbUrl.kind}, ping us ;)`
          )
        )
      if (
        typeof err === "object" &&
        "message" in err &&
        err.message.indexOf('"pg_stat_statements" does not exist')
      )
        logger.log(
          chalk.blue(
            `Can't get query history as pg_stat_statements is not enabled. Enable it for a better db analysis.`
          )
        )
      return []
    })
  const rules: Record<RuleId, RuleAnalyzed> = analyzeDatabase(
    conf,
    now,
    db,
    queries,
    history,
    referenceReport?.analysis,
    opts.only?.split(",") || []
  )
  const [offRules, usedRules] = partition(
    Object.values(rules),
    (r) => r.conf.level === RuleLevel.enum.off
  )
  const rulesByLevel: Record<RuleLevel, RuleAnalyzed[]> = groupBy(
    usedRules,
    (r) => r.conf.level
  )
  ...
  if (opts.email) {
    ...
    if (opts.key) {
      ...
      logger.log(
        chalk.blue(
          `For any question or suggestion, reach out to ${azimuttEmail}.`
        )
      )
      ...
      logger.log(
        chalk.blue(
          "Get even more from it with a license key, enabling historical analysis to identify:"
        )
      )
    ...
    logger.log(
      chalk.blue(
        "Add your professional email (ex: `--email your.name@company.com`) to get the full report in JSON."
      )
    )
    logger.log(
      chalk.blue(
        `Reach out to ${azimuttEmail} for feedback or suggest improvements ;)`
      )
    )
    logger.log(chalk.blue(`Cheers!`))
    logger.log("")
...
function isValidEmail(
  ...
  if (parsed.domain) {
    if (parsed.domain === "azimutt.app") {
      logger.log(
        chalk.red(`Do you really have an 'azimutt.app' email? Good try ;)`)
      )
      return false
      ...
      logger.log(
        chalk.red(
          `Got email param, please use your professional one instead ;)`
        )
      )
      return false
    } else {
    ...
    logger.log(
      chalk.red(`Unrecognized email (${email}), try adding quotes around it.`)
    )
    return false
  }
}
...
function isValidKey(
  ...
  if (!email) {
    ...
    logger.log(
      chalk.red(
        `Unrecognized key for ${email}, reach out to ${azimuttEmail} for help.`
      )
    )
    track(
      "cli__analyze__run",
...
async function loadConf(folder: string, logger: Logger): Promise<RulesConf> {
  ...
  if (fileExists(path)) {
    ...
    return await fileReadJson<RulesConf>(path).then(
      zodParseAsync(RulesConf, `RulesConf reading ${path}`)
    )
  } else {
    mkParentDirs(path)
...
function buildStats(
  ...
  rulesByLevel: Record<RuleLevel, RuleAnalyzed[]>
): AnalyzeStats {
  const violationsByLevel: Record<RuleLevel, number> = mapValues(
    rulesByLevel,
    (rules) => rules.reduce((acc, rule) => acc + rule.violations.length, 0)
  )
  return {
    ...
    nb_rules: Object.values(rulesByLevel).reduce(
      (acc, rules) => acc + rules.length,
      0
    ),
    nb_violations: Object.values(violationsByLevel).reduce(
      (acc, count) => acc + count,
      0
    ),
...
function printReport(
  ...
  if (offRules.length > 0) {
    logger.log(
      `${pluralizeL(offRules, "off rule")}: ${offRules.map((r) => r.rule.name).join(", ")}`
    )
  }
  ruleLevelsShown
    .slice()
    ...
    .forEach((level) => {
      ...
      logger.log(
        `${levelViolationsCount} ${level} violations (${pluralizeL(levelRules, "rule")}):`
      )
      levelRules.forEach((rule) => {
        const ignores = ruleIgnores(rule)
        logger.log(
          `  ${rule.violations.length} ${rule.rule.name}${ignores}${rule.violations.length > 0 ? ":" : ""}`
        )
  ...
  logger.log("")
  logger.log(
    `Found ${pluralize(stats.nb_entities, "entity")}, ${pluralize(stats.nb_relations, "relation")}, ${pluralize(stats.nb_queries, "query")} and ${pluralize(stats.nb_types, "type")} on the database.`
  )
  logger.log(
    `Found ${stats.nb_violations} violations using ${stats.nb_rules} rules: ${ruleLevelsShown.map((l) => `${stats.violations[l] || 0} ${l}`).join(", ")}.`
  )
  logger.log("")
}
...
async function writeReport(
  ...
  const path = pathJoin(
    folder,
    `report_${dateToIsoFilename(new Date())}.azimutt.json`
  )
...
async function loadReferenceReport(
  folder: string,
  report: string,
  logger: Logger
): Promise<AnalyzeReport> {
  const path = pathJoin(folder, report)
  const res = await fileReadJson<AnalyzeReport>(path).then(
    zodParseAsync(AnalyzeReport)
  )
...
async function loadHistory(
  folder: string,
  logger: Logger
): Promise<AnalyzeHistory[]> {
  const files = await fileList(folder)
  const history = files
    .map((file) => {
      const [, date] =
        file.match(/^report_([0-9-TZ]{24})\.azimutt\.json$/) || []
      return date
        ? {
            date: dateFromIsoFilename(date).getTime(),
            path: pathJoin(folder, file),
          }
        : undefined
    })
