...
describe('stats', () => {
    ...
    test.skip('getTableStats', async () => {
        const stats = await connect(application, url, getTableStats('SalesLT.Customer'), {logQueries: true, logger})
        console.log('getTableStats', stats)
        expect(stats.rows).toEqual(847)
    })
    test.skip('getColumnStats', async () => {
        const stats = await connect(application, url, getColumnStats({table: 'SalesLT.Customer', column: 'FirstName'}), {logQueries: true, logger})
