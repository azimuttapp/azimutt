...
describe('postgresParser', () => {
    ...
    test.skip('other structures', async () => {
        ...
        const structures = [
            ...
            // 'https://raw.githubusercontent.com/bocoup/devstats/refs/heads/master/structure.sql', // fail#106: ` WITH label_fields AS (` (common table expresison)
            ...
            // 'https://raw.githubusercontent.com/Rotabot-io/rotabot/refs/heads/main/assets/structure.sql', // fail#129: `COPY public.members (id, rota_id, user_id, metadata, created_at, updated_at) FROM stdin;\.`
    ...
    describe('alterFunction', () => {
        test('rename', () => {
            expect(parsePostgresAst('ALTER FUNCTION gen_id RENAME TO gen_uid;')).toEqual({result: {statements: [{
                ...stmt('AlterFunction', 0, 13, 39),
                name: identifier('gen_id', 15),
                args: [],
                actions: [{...kind('Rename', 22, 30), name: identifier('gen_uid', 32)}],
            }]}})
        })
        test('setOwner', () => {
            expect(parsePostgresAst('ALTER FUNCTION public.gen_id(size integer) OWNER TO postgres;')).toEqual({result: {statements: [{
                ...stmt('AlterFunction', 0, 13, 60),
                schema: identifier('public', 15),
                name: identifier('gen_id', 22),
                args: [{name: identifier('size', 29), type: {name: identifier('integer', 34), token: token(34, 40)}}],
                actions: [{...kind('SetOwner', 43), owner: {kind: 'User', name: identifier('postgres', 52)}}],
            }]}})
        })
        test('setSchema', () => {
            expect(parsePostgresAst('ALTER FUNCTION public.gen_id(size integer) SET SCHEMA utils;')).toEqual({result: {statements: [{
                ...stmt('AlterFunction', 0, 13, 59),
                schema: identifier('public', 15),
                name: identifier('gen_id', 22),
                args: [{name: identifier('size', 29), type: {name: identifier('integer', 34), token: token(34, 40)}}],
                actions: [{...kind('SetSchema', 43, 52), schema: identifier('utils', 54)}],
            }]}})
        })
    })
    describe('alterMaterializedView', () => {
        test('rename', () => {
            expect(parsePostgresAst('ALTER MATERIALIZED VIEW admins RENAME TO mv_admins;')).toEqual({result: {statements: [{
                ...stmt('AlterMaterializedView', 0, 22, 50),
                name: identifier('admins', 24),
                actions: [{...kind('Rename', 31, 39), name: identifier('mv_admins', 41)}],
            }]}})
        })
        test('setOwner', () => {
            expect(parsePostgresAst('ALTER MATERIALIZED VIEW public.admins OWNER TO postgres;')).toEqual({result: {statements: [{
                ...stmt('AlterMaterializedView', 0, 22, 55),
                schema: identifier('public', 24),
                name: identifier('admins', 31),
                actions: [{...kind('SetOwner', 38), owner: {kind: 'User', name: identifier('postgres', 47)}}],
            }]}})
        })
        test('setSchema', () => {
            expect(parsePostgresAst('ALTER MATERIALIZED VIEW public.admins SET SCHEMA admin;')).toEqual({result: {statements: [{
                ...stmt('AlterMaterializedView', 0, 22, 54),
                schema: identifier('public', 24),
                name: identifier('admins', 31),
                actions: [{...kind('SetSchema', 38, 47), schema: identifier('admin', 49)}],
            }]}})
        })
    })
    describe('alterSchema', () => {
        test('rename', () => {
            expect(parsePostgresAst('ALTER SCHEMA cms RENAME TO marketing;')).toEqual({result: {statements: [{
                ...
                actions: [{...kind('Rename', 17, 25), name: identifier('marketing', 27)}]
            }]}})
        })
        test('setOwner', () => {
            expect(parsePostgresAst('ALTER SCHEMA cms OWNER TO CURRENT_USER;')).toEqual({result: {statements: [{
                ...
                schema: identifier('cms', 13),
                actions: [{...kind('SetOwner', 17), owner: kind('CurrentUser', 26, 37)}]
            }]}})
        })
    })
    ...
    describe('alterType', () => {
        test('rename', () => {
            expect(parsePostgresAst('ALTER TYPE public.status RENAME TO bug_status;')).toEqual({result: {statements: [{
                ...stmt('AlterType', 0, 9, 45),
                schema: identifier('public', 11),
                name: identifier('status', 18),
                actions: [{...kind('Rename', 25, 33), name: identifier('bug_status', 35)}],
            }]}})
        })
        test('setOwner', () => {
            expect(parsePostgresAst('ALTER TYPE public.bug_status OWNER TO postgres;')).toEqual({result: {statements: [{
                ...stmt('AlterType', 0, 9, 46),
                schema: identifier('public', 11),
                name: identifier('bug_status', 18),
                actions: [{...kind('SetOwner', 29), owner: {kind: 'User', name: identifier('postgres', 38)}}],
            }]}})
        })
        test('setSchema', () => {
            expect(parsePostgresAst('ALTER TYPE public.bug_status SET SCHEMA issues;')).toEqual({result: {statements: [{
                ...stmt('AlterType', 0, 9, 46),
                schema: identifier('public', 11),
                name: identifier('bug_status', 18),
                actions: [{...kind('SetSchema', 29, 38), schema: identifier('issues', 40)}],
            }]}})
        })
    })
    describe('alterView', () => {
        test('rename', () => {
            expect(parsePostgresAst('ALTER VIEW admins RENAME TO v_admins;')).toEqual({result: {statements: [{
                ...stmt('AlterView', 0, 9, 36),
                name: identifier('admins', 11),
                actions: [{...kind('Rename', 18, 26), name: identifier('v_admins', 28)}],
            }]}})
        })
        test('setOwner', () => {
            expect(parsePostgresAst('ALTER VIEW public.admins OWNER TO postgres;')).toEqual({result: {statements: [{
                ...stmt('AlterView', 0, 9, 42),
                schema: identifier('public', 11),
                name: identifier('admins', 18),
                actions: [{...kind('SetOwner', 25), owner: {kind: 'User', name: identifier('postgres', 34)}}],
            }]}})
        })
        test('setSchema', () => {
            expect(parsePostgresAst('ALTER VIEW public.admins SET SCHEMA admin;')).toEqual({result: {statements: [{
                ...stmt('AlterView', 0, 9, 41),
                schema: identifier('public', 11),
                name: identifier('admins', 18),
                actions: [{...kind('SetSchema', 25, 34), schema: identifier('admin', 36)}],
            }]}})
        })
    })
    describe('begin', () => {
