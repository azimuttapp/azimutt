...
export const getTableStats =
  (ref: EntityRef) =>
  async (conn: Conn): Promise<ConnectorEntityStats> => {
...
export const getColumnStats =
  (ref: AttributeRef) =>
  async (conn: Conn): Promise<ConnectorAttributeStats> => {
    const sqlTable = buildSqlTable(ref)
...
async function getSampleValues(
  conn: Conn,
  sqlTable: SqlFragment
): Promise<{ [attribute: string]: AttributeValue }> {
  ...
  const samples = await Promise.all(
    result.fields.map(async (field, fieldIndex) => {
      const values = shuffle(
        result.rows.map((row) => row[fieldIndex]).filter((v) => !!v)
      )
      const value = await (values.length > 0
        ? Promise.resolve(values[0])
        : getSampleValue(conn, sqlTable, buildSqlColumn([field.name])))
...
async function getSampleValue(
  conn: Conn,
  sqlTable: SqlFragment,
  sqlColumn: SqlFragment
): Promise<AttributeValue> {
  // select several raws to and then shuffle results to avoid showing samples from the same raw
  const sql = `SELECT ${sqlColumn} AS value FROM ${sqlTable} WHERE ${sqlColumn} IS NOT NULL FETCH FIRST 10 ROWS ONLY`
  const rows = await conn.query(sql)
  ...
  const rows = await conn.query(
    `
        ...
        FROM 
            all_tab_columns
        WHERE
            table_name = :table_name AND column_name = :column_name ${ref.schema ? ` AND owner = :owner` : ""}`,
    [ref.entity, ref.attribute[0], ref.schema].filter(Boolean)
  )
...
async function getColumnBasics(
  conn: Conn,
  sqlTable: SqlFragment,
  sqlColumn: SqlFragment
): Promise<ColumnBasics> {
  const queries = [
    `SELECT count(*) FROM ${sqlTable}`,
    `SELECT count(*) FROM ${sqlTable} WHERE ${sqlColumn} IS NULL`,
    `SELECT count(distinct ${sqlColumn}) FROM ${sqlTable}`,
  ]
  const [rows, nulls, cardinality] = await Promise.all(
    queries.map((query) => conn.query(query).then((res) => Number(res[0][0])))
  )
...
async function getCommonValues(
  conn: Conn,
  sqlTable: SqlFragment,
  sqlColumn: SqlFragment
): Promise<ConnectorAttributeStatsValue[]> {
  const sql = `SELECT ${sqlColumn} AS value, count(*) FROM ${sqlTable} GROUP BY ${sqlColumn} ORDER BY count(*) DESC FETCH FIRST 10 ROWS ONLY`
