...
import {ConnectorSchemaOpts, parseDatabaseUrl} from "@azimutt/models";
...
describe('integration', () => {
    const url = parseDatabaseUrl('jdbc:oracle:thin:sys/oracle@//localhost:1521/FREE')
    ...
    test.skip('getSchema', async () => {
        ...
        expect(schema.entities
            ?.find((t) => t.name === 'C##AZIMUTT.USERS')?.attrs
            .find((c) => c.name === 'item_type')?.stats?.distinctValues
        ).toEqual(['ID', 'NAME'])
    })
})
