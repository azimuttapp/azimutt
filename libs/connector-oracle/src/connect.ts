import {Connection, ConnectionAttributes, getConnection, SYSDBA,} from "oracledb";
...
export async function connect<T>(application: string, url: DatabaseUrlParsed, exec: (c: Conn) => Promise<T>, opts: ConnectorDefaultOpts): Promise<T> {
    ...
    const conn: Conn = {
        ...
        queryArrayMode(sql: string, parameters: any[] = [], name?: string): Promise<QueryResultArrayMode> {
            return logQueryIfNeeded(queryCpt++, name, sql, parameters, (sql, parameters) => {
                return client.execute(sql, parameters).then(res => {
...
async function createConnection(config: ConnectionAttributes): Promise<Connection> {
    return await getConnection(config)
}

function buildConfig(application: string, url: DatabaseUrlParsed): ConnectionAttributes {
    return {
