...
describe('queryTooSlowRule', () => {
    ...
    test('ignores', () => {
        ...
        expect(queryTooSlowRule.analyze({...ruleConf, maxMs: 100}, {}, queries).map(v => v.message)).toEqual([
            'Query 123 on users is too slow (1452 ms, SELECT * FROM users;).',
            'Query 456 on events is too slow (1212 ms, SELECT * FROM events;).',
        ])
        expect(queryTooSlowRule.analyze({...ruleConf, maxMs: 100, ignores: ['456']}, {}, queries).map(v => v.message)).toEqual([
            'Query 123 on users is too slow (1452 ms, SELECT * FROM users;).',
        ])
    })
