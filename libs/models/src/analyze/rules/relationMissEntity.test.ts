...
describe('relationMissEntity', () => {
    ...
    test('violation message', () => {
        const db: Database = {entities: [], relations: [{src: {entity: 'posts', attrs: [['author']]}, ref: {entity: 'users', attrs: [['id']]}}]}
    ...
    test('ignores', () => {
        ...
        expect(relationMissEntityRule.analyze(ruleConf, now, db, [], [], [{message: '', extra: {relation: {src: {entity: 'events', attrs: [['created_by']]}, ref: {entity: 'users', attrs: [['id']]}}}}]).map(v => v.message)).toEqual([
            'Relation posts(author)->users(id), not found entities: posts, users',
        ])
    })
})
