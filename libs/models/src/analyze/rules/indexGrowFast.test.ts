...
describe('indexGrowFast', () => {
    ...
    const conf = {...ruleConf, minSize: 10 * Mo, maxGrowthYearly: 2, maxGrowthMonthly: 0.1, maxGrowthDaily: 0.01}
    ...
    test('violation message', () => {
        ...
        expect(indexGrowFastRule.analyze(conf, now, db, [], history, []).map(v => v.message)).toEqual([
    ...
    test('ignores', () => {
        ...
        expect(indexGrowFastRule.analyze({...conf, ignores: ['posts(posts_pk)']}, now, db, [], history, []).map(v => v.message)).toEqual([
            'Index users_pk(id) on users has grown by 20% (2 Mo) since 2024-04-24 (19% monthly).',
        ])
        expect(indexGrowFastRule.analyze(conf, now, db, [], history, [{message: '', entity: {entity: 'posts'}, extra: {index: {name: 'posts_pk'}}}]).map(v => v.message)).toEqual([
            'Index users_pk(id) on users has grown by 20% (2 Mo) since 2024-04-24 (19% monthly).',
        ])
    })
})
