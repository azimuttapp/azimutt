...
describe('queryExpensiveRule', () => {
    test('ignores', () => {
        ...
        expect(queryExpensiveRule.analyze(ruleConf, {}, queries).map(v => v.message)).toEqual([
        ...
        expect(queryExpensiveRule.analyze({...ruleConf, ignores: ['456']}, {}, queries).map(v => v.message)).toEqual([
            'Query 123 on users is one of the most expensive, cumulated 543 ms (SELECT * FROM users;)'
        ])
    })
