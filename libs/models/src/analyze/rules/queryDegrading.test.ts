...
describe('queryDegrading', () => {
    ...
    test('violation message', () => {
        ...
        const previous: DatabaseQuery = {id: '123', database: 'azimutt_dev', query: 'SELECT * FROM users;', rows: 1234, exec: {count: 5, minTime: 10, maxTime: 100, sumTime: 243, meanTime: 10, sdTime: 30}}
        expect(queryDegradingRule.analyze({...ruleConf, minExec: 5, minDuration: 10, maxDegradation: 1, maxDegradationMonthly: 0.2, maxDegradationDaily: 0.1}, now, {}, [query], [{report, date: twoDaysAgo, database: {}, queries: [previous]}]).map(v => v.message)).toEqual([
    ...
    test('ignores', () => {
        ...
        expect(queryDegradingRule.analyze({...ruleConf, minExec: 5, minDuration: 10, maxDegradation: 1, maxDegradationMonthly: 0.2, maxDegradationDaily: 0.1}, now, {}, queries, history).map(v => v.message)).toEqual([
        ...
        expect(queryDegradingRule.analyze({...ruleConf, minExec: 5, minDuration: 10, maxDegradation: 1, maxDegradationMonthly: 0.2, maxDegradationDaily: 0.1, ignores: ['456']}, now, {}, queries, history).map(v => v.message)).toEqual([
            'Query 123 on users degraded mean exec time by 376% (305 ms to 1452 ms) since 2024-05-23 (188% daily, SELECT * FROM users;).',
        ])
    })
})
