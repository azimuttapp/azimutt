...
describe('entityTooLarge', () => {
    ...
    test('ignores', () => {
        ...
        expect(entityTooLargeRule.analyze({...ruleConf, max: 2}, db, []).map(v => v.message)).toEqual([
            'Entity users has too many attributes (3).',
            'Entity posts has too many attributes (4).',
        ])
        expect(entityTooLargeRule.analyze({...ruleConf, max: 2, ignores: ['posts']}, db, []).map(v => v.message)).toEqual([
            'Entity users has too many attributes (3).',
        ])
    })
