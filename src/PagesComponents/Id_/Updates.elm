module PagesComponents.Id_.Updates exposing (update)
...
import PagesComponents.Id_.Components.AmlSlidebar as AmlSlidebar
import PagesComponents.Id_.Components.ProjectTeam as ProjectTeam
import PagesComponents.Id_.Components.ProjectUploadDialog as ProjectUploadDialog
import PagesComponents.Id_.Models exposing (Model, Msg(..), ProjectSettingsMsg(..), SchemaAnalysisMsg(..))
import PagesComponents.Id_.Models.CursorMode as CursorMode
import PagesComponents.Id_.Models.DragState as DragState
...
import PagesComponents.Id_.Updates.Canvas exposing (fitCanvas, handleWheel, zoomCanvas)
import PagesComponents.Id_.Updates.Drag exposing (handleDrag)
import PagesComponents.Id_.Updates.FindPath exposing (handleFindPath)
import PagesComponents.Id_.Updates.Help exposing (handleHelp)
import PagesComponents.Id_.Updates.Hotkey exposing (handleHotkey)
import PagesComponents.Id_.Updates.Layout exposing (handleLayout)
import PagesComponents.Id_.Updates.Notes exposing (handleNotes)
import PagesComponents.Id_.Updates.ProjectSettings exposing (handleProjectSettings)
import PagesComponents.Id_.Updates.Sharing exposing (handleSharing)
import PagesComponents.Id_.Updates.Source as Source
import PagesComponents.Id_.Updates.Table exposing (hideColumn, hideColumns, hideRelatedTables, hideTable, hoverColumn, hoverNextColumn, mapTablePropOrSelected, showAllTables, showColumn, showColumns, showRelatedTables, showTable, showTables, sortColumns)
import PagesComponents.Id_.Updates.VirtualRelation exposing (handleVirtualRelation)
...
update req currentLayout now msg model =
    case msg of
        ...
        Logout ->
            B.cond (model.erd |> Maybe.mapOrElse (\e -> e.project.storage /= ProjectStorage.Browser) False) (Cmd.batch [ Ports.logout, Request.pushRoute Route.Home_ req ]) Ports.logout
