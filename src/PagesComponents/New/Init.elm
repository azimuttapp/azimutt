        ...
        ([ Ports.setMeta
         ...
         , Ports.listProjects
            ...
            ++ ((query |> Dict.get "database" |> Maybe.map (\value -> [ T.send (InitTab TabDatabase), T.sendAfter 1 (DatabaseSourceMsg (DatabaseSource.GetSchema value)) ]))
                    ...
                    |> Maybe.orElse (query |> Dict.get "sample" |> Maybe.map (\value -> T.send (InitTab TabSamples) :: (Conf.schemaSamples |> Dict.get value |> Maybe.mapOrElse (\s -> [ T.sendAfter 1 (SampleProjectMsg (ImportProject.GetRemoteFile s.url (Just s.key))) ]) [])))
