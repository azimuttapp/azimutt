...
viewNewProject currentUrl shared model =
    appShell shared.conf
        ...
        Logout
        model
        [ a [ href (Route.toHref Route.Home_) ] [ Icon.outline Icon.ArrowLeft "inline-block", text " ", text model.selectedMenu ] ]
                        ...
                        [ div [ css [ "flex justify-end" ] ]
                            (Button.white3 Tw.primary [ onClick SampleSelectDrop ] [ text "Cancel" ]
                                :: (projects
                                        |> List.find (\p -> p.id == project.id)
                                        |> Maybe.mapOrElse
                                            (\_ -> [ Link.primary3 Tw.primary [ href (Route.toHref (Route.Id_ { id = project.id })), id "sample-project-btn", css [ "ml-3" ] ] [ text "View this project" ] ])
