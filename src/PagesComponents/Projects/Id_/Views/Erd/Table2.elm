module PagesComponents.Projects.Id_.Views.Erd.Table2 exposing (viewTable2)
...
viewTable2 : ZoomLevel -> CursorMode -> String -> String -> Bool -> String -> Bool -> Int -> ErdTableProps -> ErdTable -> Html Msg
viewTable2 zoom cursorMode hoverTable hoverColumn dragging openedDropdown virtualRelation index props table =
    let
        _ =
            -- TODO: re-work model so only modified tables are updated
            Debug.log "viewTable2" table.htmlId
    ...
    div
        ...
        [ Table.table
            ...
            , state =
                { color = props.color
                , hover = B.maybe (hoverTable /= "") (TableId.fromString hoverTable |> (\( s, t ) -> { schema = s, table = t }))
