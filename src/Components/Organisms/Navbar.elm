...
admin theme model state =
    nav [ css [ Color.bg theme.color 600, Tw.border_b, Color.border theme.color 300, Tw.border_opacity_25, Bp.lg [ Tw.border_none ] ] ]
        [ div [ css [ Tw.max_w_7xl, Tw.mx_auto, Tw.px_4, Bp.lg [ Tw.px_8 ], Bp.sm [ Tw.px_6 ] ] ]
            [ div [ css [ Tw.relative, Tw.h_16, Tw.flex, Tw.items_center, Tw.justify_between, Bp.lg [ Tw.border_b, Color.border theme.color 400, Tw.border_opacity_25 ] ] ]
...
adminNavigationLink styles theme navigationActive navigationOnClick link =
    if link.text == navigationActive then
        ...
        a [ href link.url, onClick (navigationOnClick link), css ([ Tw.text_white, Tw.rounded_md, Tw.py_2, Tw.px_3, Tw.font_medium, Css.hover [ Color.bg theme.color 500, Tw.bg_opacity_75 ] ] ++ styles) ] [ text link.text ]
    ...
    div [ css [ Tw.flex_1, Tw.px_2, Tw.flex, Tw.justify_center, Bp.lg [ Tw.ml_6, Tw.justify_end ] ] ]
        [ div [ css [ Tw.max_w_lg, Tw.w_full, Bp.lg [ Tw.max_w_xs ] ] ]
            ...
            , div [ css [ Tw.relative, Tw.text_gray_400, Tu.focusWithin [ Tw.text_gray_600 ] ] ]
                ...
                , input [ type_ "search", name "search", id search.id, placeholder "Search", css [ Tw.block, Tw.w_full, Tw.bg_white, Tw.py_2, Tw.pl_10, Tw.pr_3, Tw.border, Tw.border_transparent, Tw.rounded_md, Tw.leading_5, Tw.text_gray_900, Tw.placeholder_gray_500, Tu.focusRing ( Color.white, 600 ) ( theme.color, 600 ), Bp.sm [ Tw.text_sm ] ] ] []
                ]
            ]
        ]
...
adminMobileMenuButton theme mobileMenu isOpen =
    div [ css [ Tw.flex, Bp.lg [ Tw.hidden ] ] ]
        [ button [ type_ "button", onClick mobileMenu.onClick, css [ Color.bg theme.color 600, Tw.p_2, Tw.rounded_md, Tw.inline_flex, Tw.items_center, Tw.justify_center, Color.text theme.color 200, Css.focus [ Tw.outline_none, Tw.ring_2, Tw.ring_offset_2, Color.ringOffset theme.color 600, Tw.ring_white ], Css.hover [ Tw.text_white, Color.bg theme.color 500, Tw.bg_opacity_75 ] ], ariaControls mobileMenu.id, ariaExpanded isOpen ]
...
adminNotifications theme _ =
    button [ type_ "button", css [ Tw.ml_auto, Color.bg theme.color 600, Tw.flex_shrink_0, Tw.rounded_full, Tw.p_1, Color.text theme.color 200, Tu.focusRing ( Color.white, 600 ) ( theme.color, 600 ), Css.hover [ Tw.text_white ] ] ]
...
adminProfile theme isOpen profile =
    Dropdown.dropdown { id = profile.id, direction = BottomLeft, isOpen = isOpen }
        (\m ->
            button [ type_ "button", id m.id, onClick profile.onClick, css [ Tw.ml_3, Color.bg theme.color 600, Tw.rounded_full, Tw.flex, Tw.text_sm, Tw.text_white, Tu.focusRing ( Color.white, 600 ) ( theme.color, 600 ) ], ariaExpanded m.isOpen, ariaHaspopup True ]
...
adminMobileMenu theme navigation notifications profile mobileMenu activeMenu isOpen =
    ...
    div [ css ([ Bp.lg [ Tw.hidden ] ] ++ open), id mobileMenu.id ]
        ...
        , profile
            |> M.mapOrElse
                (\p ->
                    div [ css [ Tw.pt_4, Tw.pb_3, Tw.border_t, Color.border theme.color 700 ] ]
                        [ div [ css [ Tw.px_5, Tw.flex, Tw.items_center ] ]
                            ...
                            , div [ css [ Tw.ml_3 ] ]
                                ...
                                , div [ css [ Tw.text_sm, Tw.font_medium, Color.text theme.color 300 ] ] [ text p.email ]
                                ]
                            , notifications |> M.mapOrElse (adminNotifications theme) (div [] [])
                            ]
                        , div [ css [ Tw.mt_3, Tw.px_2, Tw.space_y_1 ] ]
                            (p.links |> List.map (\link -> a [ href link.url, css [ Tw.block, Tw.rounded_md, Tw.py_2, Tw.px_3, Tw.text_base, Tw.font_medium, Tw.text_white, Css.hover [ Color.bg theme.color 500, Tw.bg_opacity_75 ] ] ] [ text link.text ]))
                        ]
                )
                (div [] [])
        ]
