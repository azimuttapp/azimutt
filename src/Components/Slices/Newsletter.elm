...
basicSlice model =
    div [ class "bg-white" ]
        [ div [ css [ "max-w-7xl mx-auto py-24 px-4", lg "py-32 px-8 flex items-center", sm "px-6" ] ]
            ...
            , div [ css [ "mt-8", lg "mt-0 ml-8" ] ]
                [ form [ method model.form.method, action model.form.url, target "_blank", rel "noopener", css [ sm "flex" ] ]
                    ...
                    , div [ css [ "mt-3 rounded-md shadow", sm "mt-0 ml-3 flex-shrink-0" ] ]
                        [ button [ type_ "submit", css [ "w-full flex items-center justify-center py-3 px-5 border border-transparent text-base font-medium rounded-md text-white bg-indigo-600", focusRing ( Color.indigo, 500 ) ( Color.white, 500 ), hover "bg-indigo-700" ] ]
                            [ text model.form.cta ]
                        ]
                    ]
...
small : Form -> Html msg
small model =
    form [ method model.method, action model.url, target "_blank", rel "noopener", css [ "mt-6 flex flex-col", lg "mt-0 justify-end", sm "flex-row" ] ]
        [ div []
        ...
        , div [ css [ "mt-2 flex-shrink-0 w-full flex rounded-md shadow-sm", sm "mt-0 ml-3 w-auto inline-flex" ] ]
            [ button [ type_ "submit", css [ "w-full bg-indigo-600 px-4 py-2 border border-transparent rounded-md flex items-center justify-center text-base font-medium text-white", hover "bg-indigo-700", focusRing ( Color.indigo, 500 ) ( Color.white, 500 ), sm "w-auto inline-flex" ] ]
                [ text model.cta ]
            ]
        ]
