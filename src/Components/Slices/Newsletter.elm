...
import Html.Attributes exposing (action, attribute, for, href, id, method, name, placeholder, rel, required, target, type_)
    ...
    div [ css [ "bg-white" ] ]
        [ div [ css [ "max-w-7xl mx-auto py-24 px-4", sm [ "px-6" ], lg [ "py-32 px-8 flex items-center" ] ] ]
            [ div [ css [ lg [ "w-0 flex-1" ] ] ]
                ...
                , p [ css [ "mt-3 max-w-3xl text-lg text-gray-500" ] ] [ text model.description ]
                ]
            , div [ css [ "mt-8", lg [ "mt-0 ml-8" ] ] ]
                [ form [ method model.form.method, action model.form.url, target "_blank", rel "noopener", css [ sm [ "flex" ] ] ]
                    [ label [ for "newsletter-email", css [ "sr-only" ] ] [ text model.form.placeholder ]
                ...
                , p [ css [ "mt-3 text-sm text-gray-500" ] ] model.legalText
                ]
            ]
        ]
...
small model =
    form [ method model.method, action model.url, target "_blank", rel "noopener", css [ "mt-6 flex flex-col", sm [ "flex-row" ], lg [ "mt-0 justify-end" ] ] ]
        [ div []
            [ label [ for "newsletter-email", css [ "sr-only" ] ] [ text model.placeholder ]
...
modelDoc =
    ...
    , legalText = [ text "We care about the protection of your data. Read our ", a [ href "#", css [ "font-medium underline" ] ] [ text "Privacy Policy." ] ]
    }
