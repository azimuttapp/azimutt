...
slice : String -> Html msg
slice url =
    div [ css [ "bg-white" ] ]
        [ div [ css [ "max-w-4xl mx-auto py-16 px-4", sm [ "px-6 py-24" ], lg [ "max-w-7xl px-8 flex items-center justify-between" ] ] ]
            ...
            , div [ css [ "mt-8 flex", lg [ "mt-0 flex-shrink-0" ] ] ]
                [ a ([ href url, css [ "flex items-center justify-center px-5 py-3 h-14 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-gradient-to-r from-green-600 to-indigo-600", hover [ "text-white from-green-700 to-indigo-700" ] ] ] ++ track (Track.openAppCta "home-cta"))
                    [ text "Explore now!" ]
