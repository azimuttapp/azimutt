...
withLabel label =
    divider "justify-center" [ span [ css [ "px-2 bg-white text-sm text-gray-500" ] ] [ text label ] ]


withIcon : Icon -> Html msg
withIcon icon =
    divider "justify-center" [ span [ css [ "bg-white px-2 text-gray-500" ] ] [ Icon.solid icon "text-gray-500" ] ]


withLabelLeft : String -> Html msg
withLabelLeft label =
    divider "justify-start" [ span [ css [ "pr-2 bg-white text-sm text-gray-500" ] ] [ text label ] ]


divider : TwClass -> List (Html msg) -> Html msg
divider position content =
    div [ css [ "relative" ] ]
        [ div [ css [ "absolute inset-0 flex items-center" ], ariaHidden True ]
            [ div [ css [ "w-full border-t border-gray-300" ] ] []
            ]
