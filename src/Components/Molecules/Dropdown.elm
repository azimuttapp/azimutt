            ...
            [ component "dropdown"
                (\opened toggleOpen ->
                    dropdown { id = "dropdown", direction = BottomRight, isOpen = opened == "dropdown" }
                        (\m -> Button.white3 Color.primary [ id m.id, ariaExpanded True, ariaHaspopup True, onClick (toggleOpen m.id) ] [ text "Options", Icon.solid ChevronDown "" ])
                ...
                (\opened toggleOpen ->
                    div [ class "flex space-x-3" ]
                        ...
                        , dropdown { id = "BottomLeft", direction = BottomLeft, isOpen = opened == "BottomLeft" }
                            (\m -> Button.white3 Color.primary [ id m.id, ariaExpanded True, ariaHaspopup True, onClick (toggleOpen m.id) ] [ text "BottomLeft", Icon.solid ChevronDown "" ])
                            (\_ -> div [] ([ "Account settings", "Support", "License" ] |> List.map (\label -> btn "" (logAction label) [ text label ])))
                        , dropdown { id = "TopRight", direction = TopRight, isOpen = opened == "TopRight" }
                            (\m -> Button.white3 Color.primary [ id m.id, ariaExpanded True, ariaHaspopup True, onClick (toggleOpen m.id) ] [ text "TopRight", Icon.solid ChevronDown "" ])
                            (\_ -> div [] ([ "Account settings", "Support", "License" ] |> List.map (\label -> btn "" (logAction label) [ text label ])))
                        , dropdown { id = "TopLeft", direction = TopLeft, isOpen = opened == "TopLeft" }
                            (\m -> Button.white3 Color.primary [ id m.id, ariaExpanded True, ariaHaspopup True, onClick (toggleOpen m.id) ] [ text "TopLeft", Icon.solid ChevronDown "" ])
                            (\_ -> div [] ([ "Account settings", "Support", "License" ] |> List.map (\label -> btn "" (logAction label) [ text label ])))
                        ]
                )
            ]
