    ...
    case msg of
        ...
        SqlSourceUploadCreate projectId source ->
            Project.create projectId (String.unique (model.projects |> List.map .name) source.name) source
                |> (\project ->
                        ( model, Cmd.batch [ Ports.createProject project, Ports.track (Track.createProject project), Request.pushRoute (Route.Id_ { id = project.id }) req ] )
                   )
        ...
        ProjectImportCreate project ->
            ( model, Cmd.batch [ Ports.createProject project, Ports.track (Track.importProject project), Request.pushRoute (Route.Id_ { id = project.id }) req ] )

        ProjectImportCreateNew id project ->
            { project | id = id, name = String.unique (model.projects |> List.map .name) project.name }
                |> (\p ->
                        ( model, Cmd.batch [ Ports.createProject p, Ports.track (Track.importProject p), Request.pushRoute (Route.Id_ { id = p.id }) req ] )
                   )
