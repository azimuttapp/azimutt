...
export interface ElmFlags {
    now: Timestamp
    conf: { env: Env, platform: Platform, role: UserRole, desktop: boolean },
...
export type GotAutoLayout = { kind: 'GotAutoLayout', nodes: DiagramNode[] }
export const GotAutoLayout = z.object({kind: z.literal('GotAutoLayout'), nodes: DiagramNode.array()}).strict()
...
export type GotLlmSqlGenerated = { kind: 'GotLlmSqlGenerated', query: SqlStatement }
export const GotLlmSqlGenerated = z.object({kind: z.literal('GotLlmSqlGenerated'), query: SqlStatement}).strict()
export type GotLlmSqlGeneratedError = { kind: 'GotLlmSqlGeneratedError', error: string }
