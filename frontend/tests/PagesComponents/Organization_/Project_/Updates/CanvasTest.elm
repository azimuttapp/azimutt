...
suite =
    describe "PagesComponents.Organization_.Project_.Updates.Canvas"
        [ describe "performZoom"
            [ test "basic" (\_ -> CanvasProps (canvasPos 0 0) 1 |> performZoom erdElem 0.5 (viewportPos 50 50) |> Tuple.first |> Expect.equal (CanvasProps (canvasPos -25 -25) 1.5))
            , test "basic round trip" (\_ -> CanvasProps (canvasPos 0 0) 1 |> performZoom erdElem 0.5 (viewportPos 50 50) |> Tuple.first |> performZoom erdElem -0.5 (viewportPos 50 50) |> Tuple.first |> Expect.equal (CanvasProps (canvasPos 0 0) 1))
            , test "complex" (\_ -> CanvasProps (canvasPos 50 20) 0.5 |> performZoom erdElem 0.1 (viewportPos 200 300) |> Tuple.first |> Expect.equal (CanvasProps (canvasPos 20 -36) 0.6))
