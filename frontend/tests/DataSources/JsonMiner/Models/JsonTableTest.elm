            ...
            [ test "basic"
                (\_ ->
                    """{"name":"id","type":"int"}"""
                        ...
                        |> Expect.equal (Ok (JsonColumn "id" "int" Nothing Nothing Nothing Nothing Nothing Nothing))
                )
            , test "full"
                (\_ ->
                    """{"name":"id","type":"int","nullable":false,"default":"0","comment":"id col","values":["a","b"]}"""
                        |> Decode.decodeString JsonTable.decodeJsonColumn
                        |> Expect.equal (Ok (JsonColumn "id" "int" (Just False) (Just "0") (Just "id col") (Nel.fromList [ "a", "b" ]) Nothing Nothing))
                )
            , test "empty nested"
                (\_ ->
                        ...
                        |> Expect.equal
                            (Ok
                                (JsonColumn "id"
                                    ...
                                    Nothing
                                    Nothing
                                    ([ JsonColumn "kind" "string" Nothing Nothing Nothing Nothing Nothing Nothing
                                     , JsonColumn "value" "number" Nothing Nothing Nothing Nothing Nothing Nothing
                                     ]
                                        |> Nel.fromList
