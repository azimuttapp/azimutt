    ...
    describe "Layout"
        [ describe "serde"
            ...
            , testEncode "empty table row"
                Layout.encode
                (Layout [] [ TableRow 1 Position.zeroGrid Size.zeroCanvas SourceId.zero (RowQuery ( "", "users" ) (Nel (ColumnMatch (Nel "id" []) DbNull) [])) (TableRow.StateSuccess (TableRow.SuccessState [] Set.empty Set.empty False Time.zero Time.zero)) False ] [] [] Time.zero Time.zero)
