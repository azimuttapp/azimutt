...
viewModal currentUrl urlOrganization shared model _ =
    Keyed.node "div"
        ...
        ([ model.modal |> Maybe.map (\m -> ( m.id, Modals.view (model.openedDialogs |> List.member m.id) m ))
         ...
         , model.sharing |> Maybe.map2 (\e m -> ( m.id, ProjectSharing.view SharingMsg ModalClose confirmDanger shared.zone currentUrl (model.openedDialogs |> List.member m.id) e m )) model.erd
