...
viewLeftSidebar model =
    let
        ...
        content =
            model.detailsSidebar |> Maybe.map2 (DetailsSidebar.view DetailsSidebarMsg (\id -> ShowTable id Nothing "details") (ShowColumn 1000) HideColumn (LLoad "fit" >> LayoutMsg) (\source q -> DataExplorer.Open (Just source) (Just q) |> DataExplorerMsg) ColorMsg DropdownToggle model.openedDropdown model.tableStats model.columnStats) model.erd
    in
...
viewBottomSheet model =
    let
        content : Maybe (Html Msg)
        content =
            Maybe.map2 (DataExplorer.view DataExplorerMsg DropdownToggle CustomModalOpen (Just >> SourceUpdateDialog.Open >> PSSourceUpdate >> ProjectSettingsMsg) (\id -> ShowTable id Nothing "data-explorer") (\i q h -> ShowTableRow i q h "data-explorer") (\t c -> NotesMsg.NOpen t c |> NotesMsg) (calcNavbarHeight model) model.openedDropdown Conf.ids.dataExplorerDialog model.dataExplorer) model.erd model.dataExplorer.display
    in
