...
viewBottomSheet model =
    let
        content : Maybe (Html Msg)
        content =
            model.dataExplorer.display |> Maybe.map2 (\erd -> DataExplorer.view DataExplorerMsg DropdownToggle CustomModalOpen (Just >> SourceUpdateDialog.Open >> PSSourceUpdate >> ProjectSettingsMsg) (\id -> ShowTable id Nothing "data-explorer") (\i q s h -> ShowTableRow i q s h "data-explorer") (\t c -> NotesMsg.NOpen t c |> NotesMsg) (calcNavbarHeight model) model.openedDropdown erd.settings.defaultSchema Conf.ids.dataExplorerDialog erd.sources (erd |> Erd.currentLayout) erd.metadata model.dataExplorer) model.erd
    in
