...
viewBottomSheet model =
    let
        content : Maybe (Html Msg)
        content =
            Maybe.map2 (DataExplorer.view DataExplorerMsg DropdownToggle CustomModalOpen (Just >> SourceUpdateDialog.Open >> PSSourceUpdate >> ProjectSettingsMsg) (\id -> ShowTable id Nothing "data-explorer") (\i q s h -> ShowTableRow i q s h "data-explorer") (\t c -> NotesMsg.NOpen t c |> NotesMsg) (calcNavbarHeight model) model.openedDropdown Conf.ids.dataExplorerDialog model.dataExplorer) model.erd model.dataExplorer.display
    in
