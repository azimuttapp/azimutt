...
view : msg -> (TableId -> Maybe ColumnPath -> msg) -> (Bool -> msg) -> msg -> Platform -> ErdConf -> SchemaName -> TableRow -> Maybe Notes -> Html msg
view refresh openNotes setCollapse delete platform conf defaultSchema row notes =
    div [ class "z-max" ]
        ([ div [ class "px-4 py-1 text-sm font-medium leading-6 text-gray-500" ] [ text (TableId.show defaultSchema row.table ++ " row") ] ]
            ++ ([ Maybe.when conf.layout { label = B.cond row.selected "Refresh selected rows" "Refresh data", content = ContextMenu.Simple { action = refresh } }
                , Maybe.when conf.layout { label = notes |> Maybe.mapOrElse (\_ -> "Update notes") "Add notes", content = ContextMenu.SimpleHotkey { action = openNotes row.table Nothing, platform = platform, hotkeys = Conf.hotkeys |> Dict.getOrElse "notes" [] } }
                , Maybe.when conf.layout
                    ...
                    , content = ContextMenu.SimpleHotkey { action = setCollapse (not row.collapsed), platform = platform, hotkeys = Conf.hotkeys |> Dict.getOrElse "collapse" [] }
                    }
