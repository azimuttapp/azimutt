...
buildSource source _ schema =
    ...
    , types = schema.types |> Dict.fromListBy .id
...
createTable tables table =
    ...
    ( { id = id
      ...
      , view = False
      , definition = Nothing
      , columns = table.columns |> Nel.toList |> List.indexedMap (createColumn table.primaryKey) |> Dict.fromListBy .name
...
createView tables statement view =
    ...
    , columns = view.select.columns |> List.indexedMap (buildViewColumn tables) |> Dict.fromListBy .name
    , primaryKey = Nothing
    , uniques = []
    , indexes = []
    , checks = []
