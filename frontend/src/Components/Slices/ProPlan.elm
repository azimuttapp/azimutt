...
projectsModalBody project close titleId =
    let
        ( color, limit ) =
            ( Tw.teal, project.organization.plan.projects |> Maybe.withDefault Feature.projects.default )
    in
    showModalBody ( project, close, color )
        ...
        [ Link.primary3 color [ href (Backend.organizationBillingUrl project.organization.id Feature.projects.name), target "_blank", rel "noopener", css [ "w-full text-base", sm [ "ml-3 w-auto text-sm" ] ] ] [ Icon.solid Icon.Sun "mr-1", text "Start trial" ]
...
layoutsWarning project =
    ...
    warning color
        ...
        [ Link.secondary3 color [ href (Backend.organizationBillingUrl project.organization.id (Feature.projectLayouts.name ++ "_warning")), target "_blank", rel "noopener" ] [ Icon.outline Icon.Sparkles "mr-1", text "Start trial" ] ]
...
layoutsModalBody project close titleId =
    ...
    showModalBody ( project, close, color )
        ...
        [ Link.primary3 color [ href (Backend.organizationBillingUrl project.organization.id Feature.projectLayouts.name), target "_blank", rel "noopener", css [ "w-full text-base", sm [ "ml-3 w-auto text-sm" ] ] ] [ Icon.solid Icon.Sparkles "mr-1", text "Start trial" ]
        , Button.white3 Tw.gray [ onClick close, css [ "mt-3 w-full text-base", sm [ "mt-0 w-auto text-sm" ] ] ] [ text "Cancel" ]
        ]
...
layoutTablesModalBody project close titleId =
    let
        ( color, limit ) =
            ( Tw.sky, project.organization.plan.layoutTables |> Maybe.withDefault Feature.layoutTables.default )
    in
    showModalBody ( project, close, color )
        Icon.Calculator
        ...
        [ Link.primary3 color [ href (Backend.organizationBillingUrl project.organization.id Feature.layoutTables.name), target "_blank", rel "noopener", css [ "w-full text-base", sm [ "ml-3 w-auto text-sm" ] ] ] [ Icon.solid Icon.CheckCircle "mr-1", text "Start trial" ]
        , Button.white3 Tw.gray [ onClick close, css [ "mt-3 w-full text-base", sm [ "mt-0 w-auto text-sm" ] ] ] [ text "Cancel" ]
        ]
...
memosModalBody project close titleId =
    let
        ( color, limit ) =
            ( Tw.amber, project.organization.plan.projectDoc |> Maybe.withDefault Feature.memos.default )
    in
    showModalBody ( project, close, color )
        Icon.Newspaper
        ...
        [ Link.primary3 color [ href (Backend.organizationBillingUrl project.organization.id Feature.memos.name), target "_blank", rel "noopener", css [ "w-full text-base", sm [ "ml-3 w-auto text-sm" ] ] ] [ Icon.solid Icon.ThumbUp "mr-1", text "Start trial" ]
        , Button.white3 Tw.gray [ onClick close, css [ "mt-3 w-full text-base", sm [ "mt-0 w-auto text-sm" ] ] ] [ text "Cancel" ]
        ]
...
groupsModalBody project close titleId =
    let
        ( color, limit ) =
            ( Tw.purple, project.organization.plan.projectDoc |> Maybe.withDefault Feature.groups.default )
    in
    showModalBody ( project, close, color )
        Icon.ViewGrid
        ...
        [ Link.primary3 color [ href (Backend.organizationBillingUrl project.organization.id Feature.groups.name), target "_blank", rel "noopener", css [ "w-full text-base", sm [ "ml-3 w-auto text-sm" ] ] ] [ Icon.solid Icon.TrendingUp "mr-1", text "Start trial" ]
        , Button.white3 Tw.gray [ onClick close, css [ "mt-3 w-full text-base", sm [ "mt-0 w-auto text-sm" ] ] ] [ text "Cancel" ]
        ]
...
colorsModalBody project update model close titleId =
    ...
    showModalBody ( project, close, color )
        ...
        [ Link.primary3 color [ href (Backend.organizationBillingUrl project.organization.id Feature.colors.name), target "_blank", rel "noopener", css [ "w-full text-base", sm [ "ml-3 w-auto text-sm" ] ] ] [ Icon.solid Icon.Fire "mr-1", text "Start trial" ]
        , Button.white3 Tw.gray [ onClick close, css [ "mt-3 w-full text-base", sm [ "mt-0 w-auto text-sm" ] ] ] [ text "Cancel" ]
        ]
...
privateLinkWarning project =
    ...
    showWarning ( project, color )
        ...
        [ Link.secondary3 color [ href (Backend.organizationBillingUrl project.organization.id Feature.projectShare.name), target "_blank", rel "noopener" ] [ Icon.outline Icon.UserGroup "mr-1", text "Join us, start trial!" ] ]
...
sqlExportWarning project =
    ...
    showWarning ( project, color )
        ...
        [ Link.secondary3 color [ href (Backend.organizationBillingUrl project.organization.id Feature.schemaExport.name), target "_blank", rel "noopener" ] [ Icon.outline Icon.TrendingUp "mr-1", text "Unleash power, start trial!" ] ]
...
analysisWarning project =
    ...
    showWarning ( project, color )
        ...
        [ Link.secondary3 color
            [ href (Backend.organizationBillingUrl project.organization.id Feature.analysis.name), target "_blank", rel "noopener" ]
...
analysisResults project items render =
    if project.organization.plan.analysis /= Feature.analysis.preview || List.length items <= 5 then
        ...
        div [ class "relative" ]
            ((items |> List.take 5 |> List.map render)
                ++ [ div [ class "absolute inset-x-0 pt-32 bg-gradient-to-t from-white text-center text-sm text-gray-500 pointer-events-none", style "bottom" "-2px" ]
                        ...
                        , a [ href (Backend.organizationBillingUrl project.organization.id (Feature.analysis.name ++ "_results")), target "_blank", rel "noopener", css [ Tw.text_500 color, "underline pointer-events-auto" ] ] [ text "start trial" ]
                        , text "."
                        ]
                   ]
            )
