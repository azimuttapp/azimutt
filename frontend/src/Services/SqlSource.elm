...
update wrap now project msg model =
    case msg of
        ...
        ParseMsg parseMsg ->
            Maybe.map2
                (\parsedSchema _ ->
                    parsingUpdate parseMsg parsedSchema
                        |> (\( parsed, message ) ->
            ...
            model.parsedSchema
                ...
                |> Maybe.map
                    (\( parsedSchema, source ) ->
                        ...
                        , Extra.batch [ T.send (model.callback ( Just parsedSchema, Ok source )), Track.sqlSourceCreated project parsedSchema source ]
                        )
                    )
                |> Maybe.withDefault ( model, Extra.none )
                        ...
                        case command of
                            Ok cmd ->
                                model.schema
                                    ...
                                    |> SqlAdapter.evolve ( statement, cmd )
