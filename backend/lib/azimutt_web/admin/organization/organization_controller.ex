defmodule AzimuttWeb.Admin.OrganizationController do
  ...
  def index(conn, _params) do
    render(conn, "index.html", organizations: Admin.list_organizations(conn |> Page.from_conn()))
  end
  ...
  def show(conn, %{"id" => organization_id}) do
    with {:ok, organization} <- Admin.get_organization(organization_id) do
      ...
      |> render("show.html",
        ...
        events: Admin.get_organization_events(organization.id, conn |> Page.from_conn(%{prefix: "events", size: 40}))
      )
    end
  end
end
